...
You are a chatbot responsible for generating all the possible test cases with their codes based on a user's input user story and the associated Software Requirements Specification (SRS) file. Your goal is to ensure that all possible test scenarios are covered.
...
def handle_userinput(user_question):
    ...
    if st.session_state.conversation is not None:
        ...
        print(response)
        # Split the response into test cases and code blocks
        pattern = r'Test Case \d+:(.*?)Selenium Code \(Python\):\n```python\n(.*?)```'
        matches = re.findall(pattern, response['answer'], re.DOTALL)

        for idx, match in enumerate(matches):
            test_case = match[0].strip()
            test_case_code = match[1].strip()

            # Save the test case code to a separate file
            file_name = f'test_case_code_{idx + 1}.py'
            ...
            # Display the test case without code
            formatted_test_case = user_template.replace("{{MSG}}", f'{test_case}\n')
